...
namespace TVR.Service.Core.Math
{
    public class QuaternionFilter
    {
        ...
        private float deltat = 0.0f;        // integration interval 
        private long lastUpdate = 0; // used to calculate integration interval
        private long Now = 0;
        ...
        {
            beta = (float)(System.Math.Sqrt(3.0f / 4.0f) * System.Math.PI * (gyroError / 180.0f));
        }

        public void Update(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
        {
            float q1 = _quat.X, q2 = _quat.Y, q3 = _quat.Z, q4 = _quat.W;   // short name local variable for readability
            float norm;
            float hx, hy, bx, bz;
